...
def get_hand_value(card1, card2, card_values)
  if is_integer?(card1)
...
def get_card_value(card1, card_values)
  if is_integer?(card1)
    return card1.to_i
  elsif is_integer?(card_values[card1.to_s])
    return card_values[card1.to_s.upcase]
  end
end

def get_card_input(string, card_values)
  puts "Enter value for #{string} card: "
  temp = gets.chomp
  if is_integer?(temp) && (temp.to_i >=2 && temp.to_i <= 10)
    return temp
  elsif card_values[temp]
    return card_values[temp]
  else
...
card1 = get_card_input("first", card_values)
card2 = get_card_input("second", card_values)
dealer = get_card_input("dealers", card_values)
